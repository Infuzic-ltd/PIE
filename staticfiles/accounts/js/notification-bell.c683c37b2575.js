...
(function () {
  ...
  var STYLE = [
    '.notif-bell-wrap{position:fixed;top:14px;right:22px;z-index:200;font-family:"Inter",system-ui,sans-serif}',
    '.notif-bell-btn{position:relative;width:40px;height:40px;border-radius:50%;border:1px solid var(--line,#e7ecf3);background:#fff;cursor:pointer;display:grid;place-items:center;box-shadow:0 2px 8px rgba(16,30,60,.1);transition:transform .15s,box-shadow .15s;padding:0}',
    '.notif-bell-btn:hover{transform:translateY(-1px);box-shadow:0 4px 14px rgba(16,30,60,.16)}',
    '.notif-bell-btn svg{width:19px;height:19px;stroke:var(--ink,#1a2332);fill:none;stroke-width:1.8}',
    ...
    '.notif-dropdown{position:absolute;top:48px;right:0;width:360px;max-width:calc(100vw - 32px);background:#fff;border-radius:14px;box-shadow:0 20px 50px rgba(8,16,36,.22);border:1px solid var(--line,#e7ecf3);overflow:hidden;display:none;flex-direction:column;max-height:480px;animation:notifDropIn .15s ease}',
    ...
    '@media(max-width:480px){.notif-bell-wrap{top:10px;right:14px}.notif-dropdown{width:calc(100vw - 24px);right:-6px}}',
  ].join('');
